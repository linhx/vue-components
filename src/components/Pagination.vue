  ...
  <nav>
    <ul v-if="pages > 0" class="pagination">
      ...
      <li v-if="withLeftEllipse" class="page-item page-ellipse">
        <!-- left page input -->
        <input v-if="leftPageInput" ref="leftInput" type="number" @keydown.esc="inputLeft" @keydown.enter="inputLeft"
          @blur="inputLeft" />
        <span v-if="!leftPageInput" class="page-link" @click="openLeftInput">...</span>
      </li>
      <!-- the buttons in the visible -->
      <li v-for="i in visiblePages" :key="i" class="page-item" :class="active(i)" @click="change(i)">
        <span class="page-link">{{ i }}</span>
      </li>
      <!-- right ellipse button -->
      <li v-if="withRightEllipse" class="page-item page-ellipse">
        <!-- right page input -->
        <input v-if="rightPageInput" ref="rightInput" type="number" @keydown.esc="inputRight"
          @keydown.enter="inputRight" @blur="inputRight" />
        <span v-if="!rightPageInput" class="page-link" @click="openRightInput">...</span>
      </li>
      <!-- last page button -->
      <li v-if="pages > 1" class="page-item" :class="active(pages)" @click="change(pages)">
